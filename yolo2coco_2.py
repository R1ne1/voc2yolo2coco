"""第二步：将标签转换为coco格式并以json格式保存，代码如下。
根路径root_path中，包含images(图片文件夹)，annos.txt(bbox标注)，classes.txt(一行对应一种类别名字),
...
# 以及annotations文件夹(如果没有则会自动创建，用于保存最后的json)
root_path = r'G:\data\cell_phone_samples\correct_images_and_labels\cellphone_labels_cut_person_and_cellphone_total' \
            r'\labels_coco_format '
# 用于创建训练集或验证集
phase = 'train'  # 需要修正
